...
- **Features**:
  ...
  - **Chapo'sHub AI Hub**: 9 AI tools in one page (tab/chip selector, FontAwesome icons) — Customer Reply, Content Generator, Social-Media Captions, Product Descriptions, Email Generator, Rewrite/Improve Text, General AI Chat, Long-Form Content, Coding Assistant — each powered by NoMask/Nemotron 3 Ultra (falls back to OpenAI, then a canned template, if unconfigured), each consuming its own point cost per generation
  - Support page builder (branded contact page)
  - Referral link system
  - Points/activity history and basic analytics dashboard
  - **Peer-to-peer (P2P) points marketplace** (see below): vendors list points for sale at a self-set NGN-per-point rate; buyers place an order and pay the vendor directly off-platform (WhatsApp / bank transfer); points transfer atomically once the vendor confirms receipt of payment — replaces the previous third-party checkout-link payment flow entirely
...
## Data Architecture
- **Data Models**: `users` (auth + points balance + referral code + `whatsapp`/`is_vendor` for the P2P marketplace), `receipts`/orders history, `activity_log` (points-consuming actions), `sessions`/JWT-based auth, `p2p_listings` + `p2p_orders` (peer-to-peer points marketplace, added `migrations/0006_p2p_vendor.sql`), `opay_receipts` (dedicated OPay receipt records, added `migrations/0003_opay_service.sql`) — see `migrations/` for exact schema. (The now-unused `webhook_events`/`whop_payments` tables from a previous third-party checkout integration remain in the schema for historical data, but nothing writes to them anymore — see the P2P Points Marketplace section below.)
...
- **Security note**: this key is only ever read server-side via `c.env.NOMASK_API_KEY` inside the Hono route running on Cloudflare's edge — it is never sent to or readable by the browser, matching the same pattern already used for `OPENAI_API_KEY` and `PAYSTACK_SECRET_KEY`.
...
|---|---|---|---|
| `content` | Content Generator | 5 | `tone` |
| `social` | Social-Media Captions | 3 | `platform` |
| `product` | Product Descriptions | 4 | — |
...
(The original `reply` tool — Customer Reply, 3 points, `tone` — still uses the dedicated `/api/ai/reply` endpoint and tone-based templates; all 9 tools share one page in the UI via a FontAwesome-icon tool-chip switcher.)
...
### Status
✅ Backend, frontend, and migration (`migrations/0005_marketplace.sql`) built and verified end-to-end locally via an extensive curl test suite (listing CRUD, upload cap enforcement, admin approve/reject, exact 90/10 purchase split, double-purchase/insufficient-points/self-purchase rejection with zero side effects, purchase-gated download 200/403, category filter, search, edit-triggers-re-moderation, soft-delete). Code committed (`4d4ae10`) and pushed to `origin/main`.

✅ **Production smoke test completed and passed (2026-08-20)** against `https://chaposhub.pages.dev`: `MARKETPLACE_BUCKET` R2 binding confirmed working (real file upload returned 200 with correct size), `marketplace_listings`/`marketplace_purchases` D1 tables confirmed present and correct (migration 0005 applied), admin role grant confirmed (fresh login after the D1 `role='admin'` UPDATE produced a JWT with `"role":"admin"` that successfully accessed `/admin/pending`), and the full listing → admin-approve → public-visibility → purchase (exact 90/10 split verified via before/after point balances: buyer 245→145, seller 245→335, matching the `/sales` endpoint's `earned:90, platformFee:10`) → double-purchase rejection (409) → purchase-gated download (200 for buyer/seller, 403 for an unrelated third party) → purchase/sales history → soft-delete (listing hidden from public browse, buyer download access preserved per the by-design "past buyers keep access" rule) flow all passed. See the "Marketplace production setup" note under Deployment below for details and known follow-ups.

## P2P Points Marketplace
Points are bought and sold directly between Chapo'sHub users through a peer-to-peer (P2P) marketplace — this **replaces** the previous third-party checkout-link payment flow (see "Removed: Whop payment integration" below for what changed and why). Implemented identically in both backends: `src/routes/p2p.ts` (Hono/D1) and `server/src/routes/p2p.ts` (Express/better-sqlite3), mounted at `/api/p2p`.

### Model
- **Vendor onboarding**: any authenticated user can apply to become a vendor via `POST /vendor/apply`, which requires a validated WhatsApp number (`users.whatsapp`) **and** a one-time `P2P_VENDOR_FEE_POINTS` onboarding fee (500 points by default; `src/lib/types.ts` / `server/src/lib/types.ts`), deducted atomically the same way any other points spend is (conditional `UPDATE ... WHERE points >= ?`, never a stale read-then-write) with a matching `points_transactions` + `activities` record on success. Insufficient balance returns `402` with the required amount; re-applying while already a vendor is free (just updates the WhatsApp number, no second charge). Once approved, `users.is_vendor` is set and the user can create listings. `GET /vendor/status` returns the caller's current vendor state plus `vendorFeePoints` (so the frontend never hardcodes the fee) and current `points`. The "Join Vendor" flow in the UI (`JoinVendorModal.tsx`) shows the fee up front and swaps to a "Buy Points" CTA when the caller's balance is insufficient.
- **Listings**: vendors set their own NGN-per-point rate and a min/max order size (bounded server-side by `P2P_MIN_RATE_NGN`/`P2P_MAX_RATE_NGN`/`P2P_MIN_LISTING_POINTS`/`P2P_MAX_LISTING_POINTS` in `src/lib/types.ts`). `GET /listings` is the public Buy-tab browse; `GET /my-listings` + `POST /listings` + `PATCH /listings/:id` (pause/resume/re-price) manage a vendor's own listings.
- **Orders & off-platform settlement**: a buyer places an order against a listing (`POST /orders`), then pays the vendor directly outside the app (WhatsApp, bank transfer, etc.) and marks the order as paid (`POST /orders/:id/mark-paid`). The vendor confirms receipt of payment (`POST /orders/:id/confirm`), which atomically transfers points from the vendor to the buyer via the same conditional `UPDATE ... WHERE points >= ?` pattern used elsewhere in the app (never read-then-write), with automatic reversal if the transfer fails partway through. Either side can cancel an unconfirmed order (`POST /orders/:id/cancel`). `GET /orders/buying` / `GET /orders/selling` list each side of a user's order history.
- **Chapo'sHub never touches the money**: all payment coordination between buyer and vendor happens off-platform; the app only ever moves points between the two accounts once the vendor explicitly confirms.

### Endpoints (`/api/p2p`, all auth required)
| Endpoint | Purpose |
|---|---|
| `GET /vendor/status` | Caller's current vendor status |
| `POST /vendor/apply` | Apply to become a vendor (requires WhatsApp number) |
| `GET /listings` | Public browse of active vendor listings |
| `GET /my-listings` | Caller's own listings (vendor-only) |
| `POST /listings` | Create a listing |
| `PATCH /listings/:id` | Edit/pause/resume a listing (owner-only) |
| `POST /orders` | Place an order against a listing (buyer) |
| `GET /orders/buying` | Caller's orders as a buyer |
| `GET /orders/selling` | Caller's orders as a vendor |
| `POST /orders/:id/mark-paid` | Buyer marks an order as paid |
| `POST /orders/:id/confirm` | Vendor confirms payment — atomic points transfer |
| `POST /orders/:id/cancel` | Cancel an unconfirmed order (either side) |

### Frontend
Full Buy / Sell / Orders tabbed UI in `client/src/pages/app/Points.tsx`, styled to match the SlipCraft-inspired design language, with a dedicated `JoinVendorModal` for vendor onboarding on the Sell tab.
...
- The `WHOP_WEBHOOK_SECRET`/`WHOP_STARTER_PLAN_ID` env bindings are no longer read anywhere.
- The `webhook_events`/`whop_payments` D1 tables are left in the schema (harmless, preserves historical data from when the integration was live) but nothing writes to them anymore.
- `POST /api/points/purchase` (the older Stripe-backed package-purchase endpoint) still exists for reference but returns `501` since `STRIPE_SECRET_KEY` has never been configured — the in-app "Buy Points" screen now points users to the P2P marketplace instead of any package checkout.
...
- **Marketplace production setup (✅ verified 2026-08-20)**: all three setup items are now directly confirmed working against `https://chaposhub.pages.dev` using disposable test accounts: (1) the `MARKETPLACE_BUCKET` R2 binding is a working plain Object Storage bucket — the earlier ambiguous "R2 Data Catalog" screenshot is moot, since a real file upload/download round-tripped successfully; (2) `migrations/0005_marketplace.sql` is applied to production D1 — confirmed via a live listing insert/read; (3) the admin role grant works as designed, with one gotcha re-confirmed: **granting `role='admin'` via D1 SQL does not retroactively update already-issued JWTs** — the account must log in again after the grant for the new role to appear in its token. A full smoke test (listing → upload → admin-approve → browse → purchase with exact 90/10 split → double-purchase rejection → purchase-gated download → history → soft-delete) passed end-to-end; see the Status line in the Scripts Marketplace section above for the full result list.
  - **Follow-up flagged for the account owner**: the admin review queue currently has one **real, non-test pending listing** — "Modern SaaS Landing Page Template" by user `chapo`, 500 points, category `business`, uploaded 2026-08-19 — awaiting a genuine moderation decision (approve/reject) from the account owner; it was deliberately left untouched during the smoke test.
  - **Cleanup note**: the smoke test created three disposable accounts still present in the production `users` table — `mkttest_seller_*`, `mkttest_buyer_*`, and `mkttest_admin_*@chaposhubtest.com` (the last one still holds `role='admin'`) — plus one soft-deleted test listing and its still-live R2 object (`smoke-test-template.zip`, intentionally preserved so the test buyer keeps download access, per the "past buyers keep access after soft-delete" design). None of this is harmful, but the account owner may want to demote/delete the test accounts via the D1 console for hygiene.
- **Last Updated**: 2026-09-09 — Fixed a **404 error on Login/Sign Up when the client is deployed standalone to a non-Cloudflare-Pages host** (e.g. a Vercel preview building only `client/`): the API client's relative `/api/...` base URL only resolves correctly when the SPA is served from the same origin as the backend (`chaposhub.pages.dev` in production, or `localhost` behind the Vite dev proxy locally). `client/src/api/client.ts` now detects the current hostname at runtime and, on any other host, automatically targets the live Cloudflare Pages backend (`https://chaposhub.pages.dev/api`) directly — no dashboard reconfiguration needed. Also added `vercel.json` (repo root and `client/`) with an `/api/*` rewrite to the same backend as defense-in-depth, and a `VITE_API_BASE_URL` build-time env override for pointing a given deployment at a different backend if ever needed. Previous entry: 2026-09-09 — Removed the Whop payment webhook integration entirely and replaced it with a peer-to-peer (P2P) points marketplace (vendor onboarding, listings, off-platform-settled orders — see dedicated section above); added a real "Join Vendor" flow and a SlipCraft-styled Settings page to the bottom nav; replaced every emoji icon site-wide (client + both backend trees) with FontAwesome 6.4.0 class-based icons; added a "Join Our Community" Telegram CTA to the landing page, footer, and Settings. Previous entry: 2026-08-20 — Added the **Scripts Marketplace** feature (see dedicated section above) and merged a user-supplied landing-page redesign: switched the entire site's accent color theme from indigo to green, added real platform-brand SVG icons and a polished gradient logo mark to the landing page, and added an aspirational growth-stats bar (explicitly flagged as placeholder, not audited figures) per user approval. Previous entry: 2026-08-19 — Replaced the OPay receipt-generator UI with a full **OPay Wallet Demo** (`/services/opay`): dashboard, Send Money, To Bank, and a fully-featured History view (summary totals, filter chips, search, date grouping, tap-to-view receipt modal), gated behind Chapo'sHub login, each user with their own private simulated wallet + transaction history, every send/transfer costing real points via an atomic two-ledger deduction with automatic refund-on-failure. The existing Paystack real bank-list/resolve endpoints were reused unchanged for the To Bank flow. The legacy receipt-generator backend was intentionally kept dormant (not deleted) for potential future reuse.
