...
### Legacy OPay receipt-generator backend (dormant, kept for now)
The original form-based receipt-generator backend from Phase 1 (`src/routes/opay.ts` — `POST /generate`, `GET /history`, `GET /receipt/:id`, backed by the `opay_receipts` table) is still mounted at the same `/api/services/opay` prefix (on disjoint sub-paths, so it can't conflict with the new wallet routes) but is **no longer reachable from any UI element** — the page it used to power has been fully replaced by the wallet demo above. It has been left in place rather than deleted, in case a future "download a receipt for a wallet transaction" feature wants to reuse it; let me know if you'd rather it be removed outright.
...
- **Last Updated**: 2026-08-19 — Replaced the OPay receipt-generator UI with a full **OPay Wallet Demo** (`/services/opay`): dashboard, Send Money, To Bank, and History views, gated behind Chapo'sHub login, each user with their own private simulated wallet + transaction history, every send/transfer costing real points via an atomic two-ledger deduction with automatic refund-on-failure (verified for both the insufficient-points and insufficient-wallet-balance paths). The existing Paystack real bank-list/resolve endpoints were reused unchanged for the To Bank flow. All new backend paths tested locally via direct API calls; production D1 migration + optional `PAYSTACK_SECRET_KEY` secret still pending (see above), and code has not yet been pushed to `main` for this change.
