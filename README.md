...
### Model
- **Vendor onboarding**: any authenticated user can apply to become a vendor via `POST /vendor/apply`, which requires a validated WhatsApp number (`users.whatsapp`). Once approved, `users.is_vendor` is set and the user can create listings. `GET /vendor/status` checks the caller's current vendor state. The "Join Vendor" flow in the UI (`JoinVendorModal.tsx`) is the frontend for this.
